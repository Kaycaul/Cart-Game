...
func _process(_delta):
	...
	$Trail.position.x = (move_speed - DEFAULT_SPEED)/(MAX_SPEED-DEFAULT_SPEED)
	$Trail.position.y = position.y - side_movement -0.5
...
func _physics_process(delta):
	...
	if move_speed < MAX_SPEED+(5*coins_collected):
		move_speed += accel
